...
The Mighty-Pixel layout requires dense sensor placement to minimize dead areas. At the same time, the use of high-voltage monolithic sensors creates electrical constraints. The resulting positioning problem is therefore not only mechanical, but also electrical.\par
There are two main critical interfaces. The first is the gap between neighbouring sensor. The second is the interface between the high-voltage sensor and the conductive support structure, especially carbon-fibre-based materials. Both interfaces are relevant for the coating studies performed in this thesis.
%
...
Since high voltage is present at the sensor backside and diced edges, two uninsulated neighbouring sensors at different electric potentials separated by a small air gap can form a high-field configuration. 
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/PaschenCurveAir.png}
    \caption{Caption}
